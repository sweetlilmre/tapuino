...
#if _USE_RPATH
	if (*path == '/') { /* There is a heading separator */
		path++;
    dj->sclust = 0;		/* Strip it and start from the root dir */
	} else {							/* No heading saparator */
		dj->sclust = fs->cdir;	/* Start from the current dir */
	}
#else
	if (*path == '/')	/* Strip heading separator if exist */
		path++;
...
)
{
	...
	if (!fs) {				/* Check file system */
		...
		dj->fn = sp;
		if (*path == '.' && *(path + 1) == '\0' && fs->cdir == 0 ) {
			res = follow_path(dj, dir, "/");		/* special handling for '.' when current directory is root */
		} else {
...
)
{
    ...
    dj.fn = sp;
	  ...
	  if (res == FR_OK) {					/* Follow completed */
		  if (!dir[0]) {
			  fs->cdir = 0;			/* No entry (root dir) */
		  } else {
			  if (dir[DIR_Attr] & AM_DIR)	/* Reached to the dir */
				  fs->cdir = LD_CLUST(dir);
			  else
